...
class DenseNet(nn.Module):
    ...
    def __init__(self, tgt_modalities, growth_rate=12, block_config=(2, 2, 2), compression=0.5,
        ...
        for k in tgt_modalities:
            # self.tgt[k] = torch.nn.Linear(891, 1) # config 1
            self.tgt[k] = torch.nn.Linear(621, 1) # config 2
            # self.tgt[k] = torch.nn.Sequential(
            #         torch.nn.Linear(621, 256),
            #         torch.nn.ReLU(),
            #         torch.nn.Linear(256, 1)
            #     )
        # self.tgt = torch.nn.Linear(621, 3)
    ...
    def test_size(self):
        case = torch.ones((1, 1, 128, 128, 128))
        output = self.forward(case)
        return output.shape[1]


if __name__ == "__main__":
    model = DenseNet(tgt_modalities=['NC', 'MCI', 'DE'])
    print(model)
