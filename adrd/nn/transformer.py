...
basedir = '/data_1/skowshik/densenet_ckpt' 
t1_ckpt_path = f'{basedir}/bn_size_4/ckpt_densenet_t1_3way_config2.pt'
t2_ckpt_path = f'{basedir}/bn_size_4/ckpt_densenet_t2_3way_config2.pt'
flair_ckpt_path = f'{basedir}/bn_size_4/ckpt_densenet_flair_3way_config2.pt'
other_ckpt_path = f'{basedir}/bn_size_2/ckpt_densenet_other_3way_config2.pt'
...
class Transformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if "vit" in self.img_net.lower() or "swinunetr" in self.img_net.lower() :
        ...
        elif "densenet" in self.img_net.lower():
            self.t1_model = nn.ImagingModelWrapper(arch=self.img_net, ckpt_path=t1_ckpt_path, train_backbone=self.train_imgnet, out_dim=self.d_model, device=self.device, tgt_modalities=self.tgt_modalities)
            self.t2_model = nn.ImagingModelWrapper(arch=self.img_net, ckpt_path=t2_ckpt_path, train_backbone=self.train_imgnet, out_dim=self.d_model, device=self.device, tgt_modalities=self.tgt_modalities)
            self.flair_model = nn.ImagingModelWrapper(arch=self.img_net, ckpt_path=flair_ckpt_path, train_backbone=self.train_imgnet, out_dim=self.d_model, device=self.device, tgt_modalities=self.tgt_modalities)
            self.other_model = nn.ImagingModelWrapper(arch=self.img_net, ckpt_path=other_ckpt_path, train_backbone=self.train_imgnet, out_dim=self.d_model, device=self.device, tgt_modalities=self.tgt_modalities)
        for k, info in src_modalities.items():
            ...
            if info['type'] == 'categorical':
                ...
                # torch.nn.Sequential(
                #     torch.nn.Embedding(info['num_categories'], d_model)
                # )
                ...
                if self.img_net:
                    ...
                    # self.modules_emb_src[k] = nn.ImagingModelWrapper(arch=self.img_net, img_size=self.img_size, patch_size=self.patch_size, ckpt_path=self.imgnet_ckpt, train_backbone=self.train_imgnet, layers=self.layers, out_dim=self.d_model, device=self.device)
                    if "vit" in self.img_net.lower() or "swinunetr" in self.img_net.lower():
                        self.modules_emb_src[k] = self.img_model
                    elif "densenet" in self.img_net.lower():
                        if "t1" in k.lower():
                            self.modules_emb_src[k] = self.t1_model
                        elif "t2" in k.lower():
                            self.modules_emb_src[k] = self.t2_model
                        elif "flair" in k.lower():
                            self.modules_emb_src[k] = self.flair_model
                        else:
                            self.modules_emb_src[k] = self.other_model
