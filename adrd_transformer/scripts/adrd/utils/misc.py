...
def get_metrics(y_true, y_pred, scores):
    ...
    with warnings.catch_warnings():
        ...
        met['Accuracy'] = acc
        met['Balanced Accuracy'] = balanced_accuracy_score(y_true, y_pred)
        met['Precision'] = prc
        # met['Balanced Precision'] = TPR / (TPR + FPR)
        # met['Micro Precision'] = precision_score(y_true, y_pred, average='micro')
        # met['Macro Precision'] = precision_score(y_true, y_pred, average='macro')
        # met['Weighted Precision'] = precision_score(y_true, y_pred, average='weighted')
...
def get_metrics_multitask(y_true, y_pred, scores):
    ''' ... '''
    if type(y_true) is dict:
        met: dict[str, dict[str, float]] = dict()
        for k in y_true.keys():
            met[k] = get_metrics(y_true[k], y_pred[k], scores[k])
    else:
        met = []
        for i in range(len(y_true[0])):
            met.append(get_metrics(y_true[:, i], y_pred[:, i], scores[:, i]))
    return met
