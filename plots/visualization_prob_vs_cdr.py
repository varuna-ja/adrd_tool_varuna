...
df1 = pd.read_csv(f'{basedir}/nacc_test_with_np_cli_stripped_prob_bs128.csv')
df2 = pd.read_csv(f'{basedir}/adni_revised_labels_stripped_prob_bs128.csv')
df3 = pd.read_csv(f'{basedir}/fhs_converted_6months_cleaned_stripped_prob_bs128.csv')
...
def normality_test(df):
    print("All: ", stats.shapiro(df.DE_prob).pvalue)
    for g in df.cdr_CDRGLOB.unique():
        # print(f'group {g}')
        group = df[df.cdr_CDRGLOB == g].DE_prob
        try:
            pvalue = stats.shapiro(group).pvalue
            print(g, pvalue < 0.05, len(group))
            print(stats.kstest(group, stats.norm.cdf).pvalue)
            print()
        except:
            continue
...
def generate_plot(df, ax, cohort, order, bp_width=0.5, strip_sz=1, strip_jitter=0.24, figu="Figure 2a", xlabel='Clinical Dementia Rating', xticks=None):
    bp = sns.boxplot(data=df, x='cdr_CDRGLOB', y='DE_prob', order=order, ax=ax, showfliers=False, width=bp_width) #, gridspec_kw={'width_ratios': [1.3, 1]})
...
generate_plot(df=df1, ax=ax, order=order, cohort='NACC', bp_width=0.42, strip_sz=3, strip_jitter=0.18, figu="Figure 2i")
annotate(df=df1, ax=ax, pairs=ax1_pairs, order=order, cohort='NACC', xy=(0.97, 0.03))


plt.tight_layout()
# plt.show()
plt.savefig('fig2i.pdf', format='pdf', dpi=300, bbox_inches='tight')

#%%
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
generate_plot(df=df2, ax=ax, order=order[:-2], cohort='ADNI', bp_width=0.26, strip_sz=5, strip_jitter=0.11, figu="Figure 2j")
annotate(df=df2, ax=ax, pairs=ax2_pairs, order=order[:-2], cohort='ADNI', xy=(0.385, 0.745))
plt.tight_layout()
# plt.show()
plt.savefig('fig2j.pdf', format='pdf', dpi=300, bbox_inches='tight')

#%%
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
generate_plot(df=df3, ax=ax, order=order[:-2], cohort='FHS', bp_width=0.26, strip_sz=5, strip_jitter=0.11, figu="Figure 2k", xlabel='',xticks=['Normal', 'Impairment', 'Dementia'])
annotate(df=df3, ax=ax, pairs=ax2_pairs, order=order[:-2], cohort='FHS', xy=(0.455, 0.76))
plt.tight_layout()
# plt.show()
plt.savefig('fig2k.pdf', format='pdf', dpi=300, bbox_inches='tight')
